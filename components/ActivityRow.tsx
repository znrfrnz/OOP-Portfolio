...
export default function ActivityRow({
  ...
  const paddedNum = String(activity.id).padStart(2, "0");
  ...
  return (
    <article className="relative py-16 md:py-24 border-t border-white/[0.09] overflow-hidden group">
      ...
      >
        ...
        {/* Code block — right column, no PDF here */}
        <div className="w-full md:w-[420px] shrink-0">
          <CodeBlock
            code={codeOverride ?? activity.codePreview}
            filename={`${activity.slug}.java`}
          />
        </div>
      </div>
